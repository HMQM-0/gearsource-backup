...
import { PurchaseOrderGateway } from "./PurchaseOrderGateway";
...
describe("<PurchaseOrderGateway />", () => {
  it("renders with statuses", () => {
    const processPayment = jest.fn();
    const wrapper = mount(
      <PurchaseOrderGateway processPayment={processPayment} />
    );
  ...
  it("simulates select and submit events", (done) => {
    ...
    const wrapper = mount(
      <PurchaseOrderGateway formRef={formRef} processPayment={processPayment} />
    );
